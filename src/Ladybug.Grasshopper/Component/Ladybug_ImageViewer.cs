    ...
    public class Ladybug_ImageViewer : GH_Component
    {
        ...
        private string CheckImg(string filePath)
        {
            ...
            if (isHDR)
            {
                ...
                if (isNewHDR)
                {
                    var radTiff = new RaTiff(filePath, tiffFile);
                    radTiff.Execute();
                    if (!File.Exists(tiffFile))
                    {
            ...
            {
                tiffFile = Path.Combine(tempPath, fileName + fileExtension);
        ...
        protected override System.Drawing.Bitmap Icon => Resources.Ladybug_Viewer_24;


        public override Guid ComponentGuid => new Guid("65377C8F-F245-4801-8D54-4564EE314003");
        ...
        private IEnumerable<double> GetPValues(string HDRImg)
        {
            ...
            if (!isHDR)
            {
                throw new ArgumentException("Input image is not Radiance HDR!");
            }
            var pvalue = new PValue(HDRImg);
